...
bool ImageProcessor::loadParameters() {
    ...
    ROS_INFO("cam1_intrinscs: %f, %f, %f, %f", cam1_intrinsics[0], cam1_intrinsics[1],
...
void ImageProcessor::initializeFirstFrame() {
    ...
    for (int code = 0; code < processor_config.grid_row * processor_config.grid_col; ++code) {
        ...
        for (int k = 0;
             k < processor_config.grid_min_feature_num && k < new_features_this_grid.size(); ++k) {
...
void ImageProcessor::predictFeatureTracking(const vector<cv::Point2f> &input_pts,
    ...
    // Intrinsic matrix.
    cv::Matx33f K(intrinsics[0], 0.0, intrinsics[2], 0.0, intrinsics[1], intrinsics[3], 0.0, 0.0,
                  1.0);
...
void ImageProcessor::trackFeatures() {
    ...
    calcOpticalFlowPyrLK(
        prev_cam0_pyramid_, curr_cam0_pyramid_, prev_cam0_points, curr_cam0_points, track_inliers,
    ...
    twoPointRansac(prev_matched_cam0_points, curr_matched_cam0_points, cam0_R_p_c, cam0_intrinsics,
                   cam0_distortion_model, cam0_distortion_coeffs, processor_config.ransac_threshold,
    ...
    twoPointRansac(prev_matched_cam1_points, curr_matched_cam1_points, cam1_R_p_c, cam1_intrinsics,
                   cam1_distortion_model, cam1_distortion_coeffs, processor_config.ransac_threshold,
...
void ImageProcessor::addNewFeatures() {
    ...
    if (matched_new_features < 5 &&
        static_cast<double>(matched_new_features) / static_cast<double>(detected_new_features) <
            0.1)
...
void ImageProcessor::undistortPoints(const vector<cv::Point2f> &pts_in, const cv::Vec4d &intrinsics,
    ...
    const cv::Matx33d K(intrinsics[0], 0.0, intrinsics[2], 0.0, intrinsics[1], intrinsics[3], 0.0,
                        0.0, 1.0);
    ...
    if (distortion_model == "radtan") {
        ...
        cv::fisheye::undistortPoints(pts_in, pts_out, K, distortion_coeffs, rectification_matrix,
                                     K_new);
    } else {
...
void ImageProcessor::drawFeaturesStereo() {
    if (debug_stereo_pub.getNumSubscribers() > 0) {
        // Colors for different features.
        Scalar tracked(0, 255, 0);        // 已跟踪的点是绿色
        Scalar new_feature(0, 255, 255);  // 新点是黄色
