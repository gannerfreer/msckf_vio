...
bool MsckfVio::loadParameters() {
    ...
    nh.param<double>("initial_covariance/extrinsic_rotation_cov", extrinsic_rotation_cov,
                     3.0462e-4);
    nh.param<double>("initial_covariance/extrinsic_translation_cov", extrinsic_translation_cov,
                     1e-4);
    ...
    ROS_INFO("initial velocity: %f, %f, %f", state_server.imu_state.velocity(0),
             state_server.imu_state.velocity(1), state_server.imu_state.velocity(2));
...
void MsckfVio::featureCallback(const CameraMeasurementConstPtr &msg) {
    ...
    if (processing_time > 1.0 / frame_rate) {
        ++critical_time_cntr;
        ROS_INFO("\033[1;31mTotal processing time %f/%d...\033[0m", processing_time,
                 critical_time_cntr);
        ...
        printf("Remove camera states time: %f/%f\n", prune_cam_states_time,
               prune_cam_states_time / processing_time);
...
void MsckfVio::mocapOdomCallback(const nav_msgs::OdometryConstPtr &msg) {
    ...
    if (publish_tf) {
        ...
        tf_pub.sendTransform(tf::StampedTransform(T_b_w_gt_tf, msg->header.stamp, fixed_frame_id,
...
void MsckfVio::processModel(const double &time, const Vector3d &m_gyro, const Vector3d &m_acc) {
    ...
    Vector3d w2 = skewSymmetric(dtime * imu_state.velocity_null + imu_state.position_null -
                                imu_state.position) *
                  IMUState::gravity;
...
void MsckfVio::addFeatureObservations(const CameraMeasurementConstPtr &msg) {
    ...
    for (const auto &feature : msg->features) {
        if (map_server.find(feature.id) == map_server.end()) {
            ...
            map_server[feature.id].observations[state_id] =
                Vector4d(feature.u0, feature.v0, feature.u1, feature.v1);
        } else {
    ...
    tracking_rate =
        static_cast<double>(tracked_feature_num) / static_cast<double>(curr_feature_num);

    return;
}

void MsckfVio::measurementJacobian(const StateIDType &cam_state_id, const FeatureIDType &feature_id,
                                   Matrix<double, 4, 6> &H_x, Matrix<double, 4, 3> &H_f,
                                   Vector4d &r) {
...
void MsckfVio::featureJacobian(const FeatureIDType &feature_id,
                               const std::vector<StateIDType> &cam_state_ids, MatrixXd &H_x,
                               VectorXd &r) {
...
void MsckfVio::measurementUpdate(const MatrixXd &H, const VectorXd &r) {
    ...
    for (int i = 0; i < state_server.cam_states.size(); ++i, ++cam_state_iter) {
        ...
        cam_state_iter->second.orientation =
            quaternionMultiplication(dq_cam, cam_state_iter->second.orientation);
...
void MsckfVio::removeLostFeatures() {
    ...
    for (const auto &feature_id : invalid_feature_ids) map_server.erase(feature_id);
    ...
    for (const auto &feature_id : processed_feature_ids) map_server.erase(feature_id);
...
void MsckfVio::findRedundantCamStates(vector<StateIDType> &rm_cam_state_ids) {
    ...
    for (int i = 0; i < 4; ++i) --key_cam_state_iter;
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (angle < rotation_threshold && distance < translation_threshold &&
...
void MsckfVio::pruneCamStateBuffer() {
    ...
    for (auto &item : map_server) {
        ...
        for (const auto &cam_id : involved_cam_state_ids) feature.observations.erase(cam_id);
    }
    ...
    for (const auto &cam_id : rm_cam_state_ids) {
        int cam_sequence =
            std::distance(state_server.cam_states.begin(), state_server.cam_states.find(cam_id));
        ...
        if (cam_state_end < state_server.state_cov.rows()) {
            ...
            state_server.state_cov.conservativeResize(state_server.state_cov.rows() - 6,
...
void MsckfVio::onlineReset() {
    ...
    ROS_INFO("Stardard deviation in xyz: %f, %f, %f", position_x_std, position_y_std,
             position_z_std);
...
void MsckfVio::publish(const ros::Time &time) {
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) odom_msg.twist.covariance[i * 6 + j] = P_body_vel(i, j);
    ...
    for (const auto &item : map_server) {
        ...
        if (feature.is_initialized) {
            ...
            feature_msg_ptr->points.push_back(
                pcl::PointXYZ(feature_position(0), feature_position(1), feature_position(2)));
        }
    }
